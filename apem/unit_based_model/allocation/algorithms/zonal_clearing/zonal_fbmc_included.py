...
from apem.unit_based_model.allocation.algorithms.fbmc_utils import (
    create_pypsa_network_from_scenario,
    fix_missing_generator_timeseries,
    calculate_nodal_ptdf,
)

from apem.unit_based_model.allocation.algorithms.zonal_clearing.zonal_fbmc import (
    BaseCaseGenerator,
    ZonalDispatchModel,
    get_zone_maps,
    aggregate_by_zone,
...
class Zonal_FBMC(PowerFlowModel):
    ...
    def create_zonal_scenario_FBMC(self, base_scenario: Scenario, results_root: Optional[str] = None) -> Scenario:
        ...
        results_path = results_root or f"results/unit_based_model/{base_scenario.name}_results/Zonal_FBMC/{self.zonal_configuration}"
