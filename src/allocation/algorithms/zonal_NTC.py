...
class Zonal_NTC(PowerFlowModel):
    ...
    def __init__(self, zonal_configuration, factor):
    ...
    def create_zonal_scenario_NTC(self, base_scenario: Scenario, network: nx.Graph, name: str) -> Scenario:
        ...
        lines = {(z1, z2): {'F_max': 0, 'B': float('inf')} for z1 in zones.keys() for z2 in zones.keys() if z1 < z2}
        ...
        for v in base_scenario.network.nodes:
            for w in base_scenario.network.nodes:
                if node_to_zone[v] < node_to_zone[w] and (v, w) in base_scenario.network.edges():
                    ...
                    if lines[node_to_zone[v], node_to_zone[w]]['B'] > base_scenario.network[v][w]['B']:
                        lines[node_to_zone[v], node_to_zone[w]]['B'] = base_scenario.network[v][w]['B']

        for z1 in zones.keys():
            for z2 in [z for z in zones.keys() if z1 < z]:
                aggregated_network.add_edge(z1, z2, B=lines[z1, z2]['B'],
