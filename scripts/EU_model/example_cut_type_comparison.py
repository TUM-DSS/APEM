"""
Example script for comparing Euphemia cut types on one EU dataset.

The script:
1. runs one EU dataset for the selected cut types, or reuses the latest matching run if enabled,
...
`EU_results/euphemia/<DATASET>/evaluation/cut_type_comparison/`
...
DATASET = EU_Datasets.TEST_3NODE
NETWORK_MODEL = "FBMC"
...
def eu_results_root(dataset: EU_Datasets) -> Path:
    """Return the canonical EU results root for one dataset."""
    return ROOT / "EU_results" / "euphemia" / dataset.name


def evaluation_root(dataset: EU_Datasets) -> Path:
    """Return the folder used for cut-type comparison outputs."""
    return eu_results_root(dataset) / "evaluation" / "cut_type_comparison"


def create_output_dir(dataset: EU_Datasets, network_model: str) -> Path:
...
def latest_matching_run(
    ...
    dataset_root = eu_results_root(dataset)
