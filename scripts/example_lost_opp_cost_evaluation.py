...
- `DATASET`: choose the US dataset to analyze.
- `POWER_FLOW_MODEL`: choose the power-flow model from `apem.US_market_model.enums.PowerFlowModels`
...
`US_results/<scenario>_results/evaluation/lost_opp_cost_comparison/`
"""
...
def build_power_flow_model():
    """Build the power-flow model lazily to match the solve_US_scenario signature."""
