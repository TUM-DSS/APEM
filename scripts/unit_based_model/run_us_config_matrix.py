"""
Utility to run a matrix of APEM unit-based-model configurations to sanity-check the pipeline.
...
from apem.unit_based_model.enums import PricingAlgorithms, RedispatchAlgorithms, UnitBased_Datasets, FBMCBaseCases
...
def run_single(
    ...
    solve_and_analyse_scenario(
        unit_based_dataset=dataset,
        order_book_based_dataset=dataset,  # unused for unit-based-model runs
        market_model=MarketModels.unit_based_model,
        power_flow_model=power_flow_model,
        cut_type=None,  # unused for unit-based-model runs
...
def _matrix() -> Iterable[Tuple[UnitBased_Datasets, Scenario]]:
    """Return the dataset list to test."""
    return [
        UnitBased_Datasets.IEEE_RTS,
        UnitBased_Datasets.PJM,
        UnitBased_Datasets.PyPSAEurSmall,
        UnitBased_Datasets.PyPSAEurLarge,
        UnitBased_Datasets.ARPA,
    ]
...
def main():
    ...
    pypsa_datasets = {UnitBased_Datasets.PyPSAEurSmall, UnitBased_Datasets.PyPSAEurLarge}
